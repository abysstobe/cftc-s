...
async function handleAdminRequest(request, config) {
  ...
  try {
    ...
    const fileCards = fileList.map(file => {
        ...
        return `
          <div class="file-card" data-url="${url}" data-category-id="${file.category_id || ''}" data-file-name="${file.file_name || ''}">
            ...
            <div class="file-info">
              ...
              <div class="original-name">原名: ${file.file_name || '无'}</div>
              <div>大小: ${formatSize(file.file_size || 0)}</div>
              <div>上传时间: ${formatDate(file.created_at)}</div>
              <div>分类: <span class="category-name">${file.category_name || '无分类'}</span></div>
              <div class="remark-display">备注: <span class="remark-text">${file.remark || '无'}</span></div>
            </div>
...
async function handleSearchRequest(request, config) {
  ...
  try {
    ...
    const files = await config.database.prepare(`
       SELECT url, fileId, message_id, created_at, file_name, file_size, mime_type, remark, category_id
       FROM files 
...
function generateAdminPage(fileCards, categoryOptions) {
  ...
  <head>
    ...
    <style>
      ...
      .delete-files-btn { background: #e74c3c; }
      .delete-category-btn { background: #e74c3c; }
      ...
      .btn-share { background: #3498db; color: white; flex: 1; }
      ...
      .modal-button:hover { transform: translateY(-2px); box-shadow: 0 4px 8px rgba(0,0,0,0.15); }
      ...
      @media (max-width: 768px) {
        ...
        .right-content, .action-bar-left, .action-bar-right { flex-direction: column; align-items: stretch; gap: 10px; width: 100%; }
        .header input.search, .header select.category-filter, .header a.return-btn { width: 100%; box-sizing: border-box; margin-left: 0; text-align: center; }
  ...
  <body>
    <div class="container">
      <div class="header">
        ...
        <div class="right-content">
          <input type="text" id="search-input" name="searchInput" class="search" placeholder="搜索新/旧文件名...">
      ...
      <div id="remarkModal" class="modal">
        <div class="modal-content">
          <h3 class="modal-title">添加/修改备注</h3>
          <textarea id="remarkInput" name="remarkInput" placeholder="输入备注信息..."></textarea>
          <div class="modal-buttons">
            <button class="modal-button modal-confirm" id="remarkConfirm">确认</button>
            <button class="modal-button modal-cancel" id="remarkCancel">取消</button>
          </div>
        </div>
      </div>
    </div>
    <script>
      ...
      setTimeout(setBingBackground, 100);
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        if (editSuffixCancel) editSuffixCancel.addEventListener('click', () => document.getElementById('editSuffixModal').classList.remove('show'));
        if (remarkFilesBtn) remarkFilesBtn.addEventListener('click', showRemarkModal);
        if (changeCategoryBtn) changeCategoryBtn.addEventListener('click', confirmChangeCategory);
        if (remarkConfirm) remarkConfirm.addEventListener('click', updateRemarkForSelected);
        if (remarkCancel) remarkCancel.addEventListener('click', () => document.getElementById('remarkModal').classList.remove('show'));
      ...
      function initializeFileCards() {
        ...
        fileGrid.querySelectorAll('.file-card').forEach(card => {
          ...
          card.addEventListener('click', (e) => {
            if (e.target.tagName === 'BUTTON' || e.target.tagName === 'A' || e.target === checkbox) return;
      ...
      function filterFiles() {
        ...
        document.querySelectorAll('.file-card').forEach(card => {
          ...
          const matchesSearch = fileName.includes(searchTerm) || originalFileName.includes(searchTerm);
        ...
        try {
            ...
            if (data.status === 1) {
                ...
                urls.forEach(url => {
                    ...
                    if (card) card.querySelector('.remark-text').textContent = remark || '无';
                });
                showConfirmModal(data.msg, null, true);
            } else {
      ...
      document.getElementById('confirmModalConfirm').addEventListener('click', handleConfirmModalConfirm);
      ...
      async function updateFileSuffix() {
        ...
        try {
          ...
          if (data.status === 1) {
            ...
            showConfirmModal(data.msg, () => window.location.reload(), true);
          } else {
      ...
      window.deleteFile = (url) => showConfirmModal('确定要删除这个文件吗？', () => deleteFile(url));
